...
class FSMVendingMachineTest extends TestKit(ActorSystem("fsm-vem-mach"))
with ImplicitSender
with WordSpecLike with BeforeAndAfterAll
 {
  ...
  import FSMVendingMachine.FSMVendingMach

  def vendMachineRequest(product: String) = {
    val vendM = system.actorOf(Props[FSMVendingMach])
  ...
  "A vending machine" should {
    "error when not initialize" in {
      val vendingMachine = system.actorOf(Props[FSMVendingMach])
